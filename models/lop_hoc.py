...
class LopHoc(BaseModel):
    def __init__(self, ma_lop: str, ten_mon: str, ma_mon: str, giang_vien: str,
                 si_so_toi_da: int = 50, phong_hoc: str = "",
                 lich_hoc: str = "", loai_hinh: str = "Lý thuyết"):
    ...
    def validate(self) -> tuple[bool, str]:
        ...
        if self.loai_hinh not in ("Lý thuyết", "Thực hành", "Online"):
    ...
    def from_dict(cls, data: dict) -> "LopHoc":
        obj = cls(
            ...
            loai_hinh   = data.get("loai_hinh", "Lý thuyết"),  # tương thích dữ liệu cũ
        )
    ...
    def __str__(self) -> str:
        return (f"[{self.ma_lop}] {self.ten_mon} | GV: {self.giang_vien} "
                f"| Sĩ số: {self.si_so_hien_tai}/{self.si_so_toi_da} | {self.loai_hinh}")
